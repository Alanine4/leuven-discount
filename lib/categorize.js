...
const RULES = [
  ...
  ['酒类',       ['bier','bière','biere','wijn','vin','alcohol','sterke drank','spirit','champagne','aperitieven','whisky','vodka','rum','tarwebier']],
...
const OVERRIDES = [
  ['零食甜点', ['roomijs']],
  // ice tea 是瓶装饮料，不能被下面「咖啡茶」的 'tea ' 认领
  ['饮料',     ['fruitsap', 'ice tea', 'iced tea', 'ijsthee']],
];
...
const PRIORITY = [
  ['乳制品奶酪', ['zuivel']],
  ['冷冻速食',   ['maaltijd']],
  ['调味酱料',   ['sauzen']],        // "Sauzen, kruiden en conserven" 别被 conserv 先撞成粮油
  // 超市只给顶层分类时酒、咖啡都埋在 "Dranken" 下，把商品名一起喂进来靠这两条拆出来。
  // 'gin '/'rum '/'thee '/'tea ' 带分隔符，免得撞 verzorging、parfum、apotheek 之类。
  ['酒类',       ['bier','wijn','champagne','cava','aperitieven','digestie','sterke drank','alcohol','whisky','gin ','vodka','rum ']],
  ['咖啡茶',     ['koffie','nespresso','dolce gusto','thee ','thee,','tea ']],
];

// 瓶身容量（40cl / 1,5L / 4x400ml）说明这是即饮饮料，"tea/thee" 指的是冰茶而不是茶叶；
// 茶叶茶包标的是袋数或克重（20st / 100 zakjes / 250g）。
const BOTTLE_SIZE = /\d+\s*(?:cl|ml)\b|\d[\d.,]*\s*l\b/;

/**
 * 超市只给顶层分类时（Carrefour、Colruyt 的酒和咖啡都挂在 "Dranken" 下）用这个：
 * 顶层已经够细就直接用；归到饮料/其他的，再把商品名拼进去撞一次关键词。
 * 顶层明说是饮料的，只接受"酒类/咖啡茶"这两个细分结果 —— 否则商品名里的
 * fruit（SPA FRUIT）、dessert（Douwe Egberts Dessert）会把饮料错拉去果蔬、零食。
 */
export function toCatWithName(topName, productName) {
  const top = toCat(topName);
  if (top !== '饮料' && top !== '其他') return top;
  const s = `${topName} ${productName}`;
  let byName = toCat(s);
  if (byName === '咖啡茶' && BOTTLE_SIZE.test(s.toLowerCase())) byName = '饮料';
  if (top === '饮料' && byName !== '酒类' && byName !== '咖啡茶') return '饮料';
  return byName;
}
