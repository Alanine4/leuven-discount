...
const html = await fetchHTML('https://www.carrefour.be/nl/al-onze-promoties?p=1');
await closeBrowser();
...
const counts = {};
$('[class]').each((_, el) => {
  for (const c of ($(el).attr('class') || '').split(/\s+/)) if (c) counts[c] = (counts[c] || 0) + 1;
});
console.log('HTML 大小:', (html.length / 1024).toFixed(0), 'KB');
console.log('\n出现 20~60 次的 class（每页 36 个商品，卡片一般落这个区间）:');
